...
nomination = ['director', 'actor', 'actress', 'actor2', 'actress2', 'original_screenplay', 'adapted_screenplay']
...
def change_director(director):
    for x in range(len(df_second['best_director'])):
        director2 = df_second.loc[x,'best_director']
        if director in director2:
            return director2
        elif director2 in director:
            return director
...
for x in range(len(nomination)):
    df_second['best_{}'.format(nomination[x])] = df_second['best_{}'.format(nomination[x])].str.lower()
    df_second['best_{}'.format(nomination[x])] = df_second['best_{}'.format(nomination[x])].apply(replacer)
    df_second['best_{}'.format(nomination[x])] = df_second['best_{}'.format(nomination[x])].apply(prob)
...
df_movies['movie'] = df_movies['movie'].apply(change)

df_second['best_director'] = df_second['best_director'].apply(change_director)
...
df_second['best_original_screenplay'] = df_second['best_original_screenplay'].apply(change_original_screenplay)
df_second['best_adapted_screenplay'] = df_second['best_adapted_screenplay'].apply(change_adapted_screenplay)

for x in range(len(nomination)):
    df_third = pd.DataFrame()
    df_third['{}'.format(nomination[x])] = df_second.pivot_table(
        index='best_{}'.format(nomination[x]), values = 'point_{}'.format(nomination[x]), aggfunc='count')
    df_third = df_third.reset_index(level='best_{}'.format(nomination[x]))
    df_third = df_third[df_third['best_{}'.format(nomination[x])] != 'zzz']
    a = df_third[['best_{}'.format(nomination[x]),'{}'.format(nomination[x])]].sort_values(
        by='{}'.format(nomination[x]), ascending=False).reset_index()
...
df_best = df_movies.pivot_table(index='movie', values='point', aggfunc='sum').reset_index(level='movie')
best_movies = df_best.groupby('movie')['point'].sum().sort_values(ascending=False).reset_index()
